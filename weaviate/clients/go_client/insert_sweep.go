...
// For each batch size:
//   1) Reset schema (dynamic if possible)
//   2) Insert MEASURE_VECS vectors starting at START_ROW
//   3) Measure SEND time = first RPC start -> last successful RPC end
//   4) Measure END-TO-END time = first RPC start -> queue drained (queue==0 AND indexing==0 stably)
//
// Env required:
...
func waitForWeaviate(base string, maxSeconds int) ([]byte, error) {
	...
	for {
		...
		if err == nil && code == 200 && len(body) > 0 {
			fmt.Printf("[CLIENT] Weaviate ready.\n")
			return body, nil
		}
...
func resetClassOnce(ctx context.Context, client *weaviate.Client, base, className string, dynamicThreshold int) error {
	...
	// Known failure mode: async indexing disabled
	msg := err.Error()
...
type ResultRow struct {
	...
	// Snapshot immediately after last successful RPC (before drain-wait).
	...
	// Snapshot at end (after drain-wait / timeout).
...
func main() {
	...
	overallSec := flag.Int("overall_sec", 6900, "overall time budget seconds")
	rpcSec := flag.Int("rpc_sec", 1800, "per-batch RPC timeout seconds")

	queueTimeoutSec := flag.Int("queue_timeout_sec", 6900, "per-batchsize queue drain timeout seconds")
	...
	for _, bs := range batchSizes {
		...
		fmt.Println("[CLIENT] ------------------------------")
		fmt.Println("[CLIENT] batch_size:", bs)
		...
		// SEND metrics are defined by last successful RPC end time.
		row.SendSec = tLastRPC - t0
		if row.SendSec > 0 {
			row.SendVPS = float64(inserted) / row.SendSec
		}

		// Snapshot right after the last successful RPC so you can quantify "queued after send".
		...
		// --------------- DRAIN LOOP ---------------
		// IMPORTANT: Even if insertion was partial, we still *measure end-to-end* as
		// "time until queue drained", because that's your definition of insertion completion.
