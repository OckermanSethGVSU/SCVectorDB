...
func makeFlatClass(className string) *models.Class {
	return &models.Class{
		Class:           className,
		Vectorizer:      "none",
...
func resetClassFlat(ctx context.Context, client *weaviate.Client, base, className string) error {
	_ = client.Schema().ClassDeleter().WithClassName(className).Do(ctx)
	cls := makeFlatClass(className)
...
func main() {
	...
	overallSec := envIntDefault("CLIENT_OVERALL_SEC", 4500)
	rpcSec := envIntDefault("CLIENT_RPC_TIMEOUT_SEC", 1800)
	queueTimeoutSec := envIntDefault("QUEUE_TIMEOUT_SEC", 4500)
	...
	fmt.Printf("[CLIENT] rank=%d/%d target=%s flat bs=%d total=%d start=%d\n",
		rank, nClients, base, batchSize, measureTotal, startRow)
	...
	// rank0 resets schema for this run
	if rank == 0 {
		fmt.Println("[CLIENT] rank0 resetting schema (flat):", className)
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
		err := resetClassFlat(ctx, client, base, className)
		cancel()
		if err != nil {
			out.FinalStatus = "error"
	...
	// Split measureTotal across ranks
	...
	for inserted < myCount {
		...
		for i := 0; i < got; i++ {
			...
			docID := fmt.Sprintf("nc_n%d_r%d_row%d_i%d", nClients, rank, cursor, i)
	...
	// Only rank0 waits for drain (avoid hammering /v1/nodes)
	if rank == 0 {
