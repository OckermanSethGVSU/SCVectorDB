...
func main() {
	outPath := flag.String("out", "index_time_pes2o_ef64.json", "output json path")
	className := flag.String("class", getenvDefault("CLASS_NAME", "PES2OEF64"), "weaviate class name")
	...
	fmt.Printf("[CLIENT] vec_dim=%d measure=%d start_row=%d batch_size=%d threshold=%d distance=cosine hnsw.ef=64 hnsw.efConstruction=100 hnsw.maxConnections=16\n",
...
// func main() {
// 	outPath := flag.String("out", "index_time_pes2o_ef64.json", "output json path")
// 	className := flag.String("class", "PES2O10MEF64", "weaviate class name")
// 	batchSize := flag.Int("batch_size", 32768, "batch size")
// 	waitSec := flag.Int("wait", 180, "seconds to wait for weaviate readiness")
// 	overallSec := flag.Int("overall_sec", 25000, "overall time budget seconds")
// 	rpcSec := flag.Int("rpc_sec", 1800, "per-batch RPC timeout seconds")
