...
WORKER_SH="${BASE_DIR}/worker_launch_ssd.sh"
...
BS_MIN_POW="${BS_MIN_POW:-2}"      # 2^2  = 4
BS_MAX_POW="${BS_MAX_POW:-15}"     # 2^15 = 32768
...
echo "[INFO] Weaviate Go insert sweep (Polaris 2-node, SSD backend)"
echo "[HOST] $(hostname)"
echo "[DATE] $(date -u)"
...
echo "[INFO] STORAGE BACKEND: SSD (/local/scratch)"
...
export GOPATH="${BASE_SCRATCH_DIR}/go-${USER}-${PBS_JOBID}"
...
# ---------- run directory (metadata on Lustre, Weaviate data on SSD) ----------
TS=$(date +"%y%m%d_%H%M")
RUN_DIR="${BASE_DIR}/sc_insert_ssd_${TS}"
...
# ---------- prepare SSD data dir on worker node ----------
SSD_DATA_DIR="/local/scratch/weaviate-${USER}-${PBS_JOBID}"
echo "[INFO] Creating SSD data dir on worker: ${SSD_DATA_DIR}"
mpirun -n 1 --ppn 1 --cpu-bind none --host "${worker_node}" \
  bash -lc "mkdir -p '${SSD_DATA_DIR}/node0'" || { echo "[FATAL] Cannot create SSD dir"; exit 10; }
...
cleanup(){
  ...
  mpirun -n 1 --ppn 1 --cpu-bind none --host "${worker_node}" \
    bash -lc "pkill -f weaviate || true; rm -rf '${SSD_DATA_DIR}' || true" >/dev/null 2>&1 || true
}
trap cleanup EXIT

echo "[INFO] launching weaviate on ${worker_node} (SSD backend)"
mpirun -n 1 --ppn 1 --cpu-bind none --host "${worker_node}" \
  bash -lc "cd '${RUN_DIR}' && env \
    SIF_PATH='${SIF_PATH}' \
    PORT='${REST_PORT}' \
    GRPC_PORT='${GRPC_PORT}' \
...
[[ -f "${BIN_PATH}" ]] || { echo "[FATAL] build failed"; tail -n 200 "${LOG_DIR}/client_build.err"; exit 12; }
...
# Move PBS logs into run dir
if [[ -n "${PBS_O_WORKDIR:-}" ]]; then
  [[ -f "${PBS_O_WORKDIR}/workflow_sc_insert_ssd.out" ]] && \
    mv "${PBS_O_WORKDIR}/workflow_sc_insert_ssd.out" "${RUN_DIR}/" 2>/dev/null || true
  [[ -f "${PBS_O_WORKDIR}/workflow_sc_insert_ssd.err" ]] && \
    mv "${PBS_O_WORKDIR}/workflow_sc_insert_ssd.err" "${RUN_DIR}/" 2>/dev/null || true
fi
