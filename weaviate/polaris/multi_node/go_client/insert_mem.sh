...
echo "[INFO] Weaviate Go insert sweep (Polaris 2-node, MEMORY backend)"
echo "[HOST] $(hostname)"
echo "[DATE] $(date -u)"
...
echo "[INFO] STORAGE BACKEND: memory (tmpfs)"
...
# ---------- run directory (metadata on Lustre, Weaviate data in RAM) ----------
TS=$(date +"%y%m%d_%H%M")
...
echo "[INFO] launching weaviate on ${worker_node} (memory backend)"
mpirun -n 1 --ppn 1 --cpu-bind none --host "${worker_node}" \
...
if [[ -n "${PBS_O_WORKDIR:-}" ]]; then
  [[ -f "${PBS_O_WORKDIR}/workflow_sc_insert_mem.out" ]] && \
    mv "${PBS_O_WORKDIR}/workflow_sc_insert_mem.out" "${RUN_DIR}/" 2>/dev/null || true
  [[ -f "${PBS_O_WORKDIR}/workflow_sc_insert_mem.err" ]] && \
    mv "${PBS_O_WORKDIR}/workflow_sc_insert_mem.err" "${RUN_DIR}/" 2>/dev/null || true
fi
