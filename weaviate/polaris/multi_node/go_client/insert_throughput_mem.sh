#!/bin/bash
#PBS -N sc_insert_throughput_mem
...
# ---------- scratch caches ----------
# Keep this consistent with worker_launch_mem.sh defaults
...
# ---------- run directory ----------
TS=$(date +"%y%m%d_%H%M")
RUN_DIR="${BASE_DIR}/sc_insert_throughput_mem_${TS}"
...
# ---------- explicit tmpfs data dir on worker ----------
...
# Pre-create and inspect worker tmpfs dir
mpirun -n 1 --ppn 1 --cpu-bind none --host "${worker_node}" bash -lc "
set -e
mkdir -p '${WEAVIATE_DATA_DIR}'
df -h /dev/shm || true
ls -ld '/dev/shm' '/dev/shm/${USER}' '/dev/shm/${USER}/${PBS_JOBID}' '${WEAVIATE_DATA_DIR}' || true
"
