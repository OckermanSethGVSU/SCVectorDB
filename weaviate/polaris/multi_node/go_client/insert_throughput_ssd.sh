#!/bin/bash
#PBS -N sc_insert_throughput_ssd
...
BS_MIN_POW="${BS_MIN_POW:-14}"
BS_MAX_POW="${BS_MAX_POW:-14}"
...
# ---------- run directory ----------
TS=$(date +"%y%m%d_%H%M")
RUN_DIR="${BASE_DIR}/sc_insert_throughput_ssd_${TS}"
...
# ---------- explicit SSD data dir on worker ----------
WEAVIATE_DATA_DIR="/local/scratch/${USER}/${PBS_JOBID}/weaviate/node0"
...
# Pre-create and inspect worker SSD dir
mpirun -n 1 --ppn 1 --cpu-bind none --host "${worker_node}" bash -lc "
set -e
mkdir -p '${WEAVIATE_DATA_DIR}'
rm -rf '${WEAVIATE_DATA_DIR:?}'/*
df -h /local/scratch || true
ls -ld '/local/scratch' '/local/scratch/${USER}' '/local/scratch/${USER}/${PBS_JOBID}' '${WEAVIATE_DATA_DIR}' || true
"
...
cleanup() {
  ...
  mpirun -n 1 --ppn 1 --cpu-bind none --host "${worker_node}" bash -lc "
    ...
    rm -rf '/local/scratch/${USER}/${PBS_JOBID}/weaviate' || true
  " >/dev/null 2>&1 || true
}
trap cleanup EXIT
