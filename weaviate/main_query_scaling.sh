#!/bin/bash
# Launch script for TASK=query_scaling.
#
# Orchestrates a three-phase pipeline on top of the Weaviate cluster that
# launchWeaviateNode.sh brings up:
#   1. insert  — drive go_client/$INSERT_BIN with UPLOAD_CLIENTS_PER_WORKER*TOTAL clients
#   2. index   — wait for object count, then index quiescence (queue==0 && shards READY)
#   3. query   — fan out one go_client/$QUERY_SCALING_BIN per client per worker with
#                partitioned query ranges, then aggregate into query_scaling_summary.json
#
# Dispatched by engine.sh::engine_main_script_path() when TASK=query_scaling.
set -euo pipefail
...
export INSERT_BIN="${INSERT_BIN:-insert_pes2o_streaming}"
export QUERY_SCALING_BIN="${QUERY_SCALING_BIN:-query_scaling}"
...
PY
        ...
        ;;
    *)
        echo "Error: main_query_scaling.sh only supports TASK=query_scaling, got '${TASK}'" >&2
        exit 1
        ;;
esac
